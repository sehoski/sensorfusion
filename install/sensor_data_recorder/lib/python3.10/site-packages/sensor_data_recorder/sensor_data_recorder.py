...
class RadarCameraNode(Node):
    def __init__(self):
        ...
        # 타이머 설정
        self.create_timer(1 / 50, self.radar_callback)  # 레이더 데이터 처리 (40Hz)
        self.create_timer(1 / 30, self.camera_callback)  # 카메라 데이터 처리 (30Hz)

    def radar_callback(self):
        try:
            batch_size = 30  # 한 번에 처리할 메시지 개수
            points = []
    ...
    def is_valid_data(self, parsed_data):
        # Noise filtering conditions - adjust as necessary
        return 0 <= parsed_data['range_m'] <= 150 and 0 <= parsed_data['speed_radial'] <= 100
